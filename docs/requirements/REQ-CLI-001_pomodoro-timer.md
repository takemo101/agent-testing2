...
**ビジネスルール**:
- BR-028: 通知には「一時停止」「停止」の2つのアクションボタンを含める
- BR-029: 通知クリック時はステータス表示を行う
- BR-030: 通知は通知センターに履歴として残る
...
**ビジネスルール**:
- BR-031: 作業中は🍅、休憩中は☕をアイコンに表示
- BR-032: 停止中はグレーアウトしたアイコンを表示
- BR-033: メニューから直接タイマー操作が可能
...
**ビジネスルール**:
- BR-034: デフォルトで「仕事」フォーカスモードを使用
- BR-035: フォーカスモード名は設定で変更可能
- BR-036: フォーカスモード連携はオプションで無効化可能
...
| システム名 | 連携内容 | 方式 | 頻度 |
|-----------|---------|------|------|
| OSネイティブ通知 | デスクトップ通知の表示 | システムAPI | タイマー完了時 |
...
| ID | リスク | 影響度 | 発生確率 | 対策 |
|----|--------|--------|---------|------|
| R-001 | 通知ライブラリがOSで動作しない | 高 | 低 | 事前に3つのOSで動作確認、フォールバック実装 |
| R-002 | ターミナルでANSI制御が効かない | 中 | 低 | シンプルなテキスト表示にフォールバック |
| R-003 | バックグラウンド実行の実装が困難 | 高 | 中 | デーモン化の代わりにプロセス永続化を検討 |
| R-004 | クロスプラットフォームビルドの複雑さ | 中 | 中 | GitHub Actionsでのクロスコンパイル自動化 |
...
| Tokio | Rustの非同期ランタイム |
| notify-rust | Rustのクロスプラットフォーム通知ライブラリ |
...
- 設定ファイルによる永続化（F-013）
- 時間追加機能（F-014）
- カラー/スタイルカスタマイズ（F-015）

### Phase 3以降で検討する機能
- ステータスバー連携（Waybar/i3blocks等）（F-016）
...
| E005 | 通知の送信に失敗しました | OS通知APIエラー | システム通知設定を確認 |
